...
public class DefaultAdminClientProvider implements AdminClientProvider {
    ...
    /**
     * Create a Kafka Admin interface instance handling the following different
     * scenarios:
     *
     * 1. No TLS connection, no TLS client authentication:
     *
     * If {@code kafkaCaTrustSet} and {@code authIdentity} are null, the returned
     * Admin Client instance
     * is configured to connect to the Apache Kafka bootstrap (defined via
     * {@code hostname}) on plain connection with no
     * TLS encryption and no TLS client authentication.
     *
     * 2. TLS connection, no TLS client authentication
     *
     * If only {@code kafkaCaTrustSet} is provided as not null, the returned Admin
     * Client instance is configured to
     * connect to the Apache Kafka bootstrap (defined via {@code hostname}) on TLS
     * encrypted connection but with no
     * TLS authentication.
     *
     * 3. TLS connection and TLS client authentication
     *
     * If {@code kafkaCaTrustSet} and {@code authIdentity} are provided as not null,
     * the returned
     * Admin Client instance is configured to connect to the Apache Kafka bootstrap
     * (defined via {@code hostname}) on
     * TLS encrypted connection and with TLS client authentication.
     */
    @Override
    public Admin createAdminClient(String bootstrapHostnames, PemTrustSet kafkaCaTrustSet,
            PemAuthIdentity authIdentity, Properties config) {
        return Admin.create(adminClientConfiguration(bootstrapHostnames, kafkaCaTrustSet,
                authIdentity, config));
    }
     ...
     * @param authIdentity       Identity for TLS client authentication for
     *                           connecting to Kafka
     * @param config             Custom Admin client configuration or empty
     *                           properties instance
     *
     * @return Admin client configuration
     */
    /* test */ static Properties adminClientConfiguration(String bootstrapHostnames, PemTrustSet kafkaCaTrustSet,
            PemAuthIdentity authIdentity, Properties config) {
        if (config == null) {
        ...
        LOGGER.infoOp("creating waimea admin client: bootstrap=" + bootstrapHostnames);
        ...
        if (kafkaCaTrustSet != null) {
            ...
            config.setProperty(SslConfigs.SSL_TRUSTSTORE_CERTIFICATES_CONFIG,
                    kafkaCaTrustSet.trustedCertificatesString());
        }
